...
from data import all_different
import util

class AllDifferentRecurrentRelationalNet(Model):
    ...
    def __init__(self, is_testing):
        ...
        with tf.Graph().as_default(), tf.device('/cpu:0'):
            ...
            self.train, self.valid, self.test = self.encode_data(all_different())
            ...
            if self.edges == 'sudoku':
                ...
                edges = [(i, j) for i in range(9) for j in range(9) if not i == j]
            else:
                raise ValueError('edges must be sudoku or full')

            edge_indices = tf.constant([(i + (b * 9), j + (b * 9)) for b in range(self.batch_size) for i, j in edges], tf.int32)
            ...
            with tf.variable_scope(tf.get_variable_scope()):
                for device_nr, device in enumerate(self.devices):
                    with tf.device('/cpu:0'):
                        ...
                        x = tf.reshape(x, (-1, self.emb_size))
            ...
            for step in range(self.n_steps):
                ...
                digit_acc = tf.reduce_mean(tf.cast(equal,  dtype=tf.float32)) 
                tf.summary.scalar('steps/%d/digit-acc' % step, digit_acc)

                puzzle_acc = tf.reduce_mean(tf.cast(tf.reduce_all(equal, axis=1), dtype=tf.float32))
            ...
            self.session.run(tf.global_variables_initializer()) #tf.compat.v1.global_variables_initializer
            self.saver = tf.train.Saver() #tf.compat.v1.train.Saver()
            #print(tf.trainable_variables())
            #print(type(tf.trainable_variables()))
            util.print_vars(tf.trainable_variables()) #tf.compat.v1.trainable_variables

            self.train_writer = tf.compat.v1.summary.FileWriter('/tmp/tensorboard/sudoku/%s/train/%s' % (self.revision, self.name), self.session.graph)
            self.test_writer = tf.compat.v1.summary.FileWriter('/tmp/tensorboard/sudoku/%s/test/%s' % (self.revision, self.name), self.session.graph) #tf.summary.FileWriter('/tmp/tensorboard/sudoku/%s/test/%s' % (self.revision, self.name), self.session.graph)
            self.summaries = tf.summary.merge_all() #tf.compat.v1.summary.merge_all 
